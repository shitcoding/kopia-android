...
cmd_help() {
    ...
    echo ""
    echo "Examples:"
    echo "  $(basename "$0") create 3          # Create 3 AVDs"
    echo "  $(basename "$0") start 3           # Start all 3 with visible windows"
    echo "  $(basename "$0") setup 3           # Install APK + push repos to all 3"
    echo "  $(basename "$0") list              # Show running e2e AVDs"
    echo ""
    echo "  # Run Maestro with 3-way sharding:"
    echo "  maestro --device \"emulator-5554,emulator-5556,emulator-5558\" test --shard-split=3 e2e/maestro/"
    echo ""
