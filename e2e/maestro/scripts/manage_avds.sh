...
cmd_help() {
    ...
    echo ""
    echo "  # Run the E2E flows (per-flow runner: prereqs + reset + retry + artifacts):"
    echo "  bash e2e/maestro/scripts/run_e2e.sh emulator-5554"
    echo "  # or fast native sharding across 2 devices (no per-flow hooks):"
    echo "  bash e2e/maestro/scripts/run_e2e.sh --shard-split emulator-5554,emulator-5556"
    echo ""
