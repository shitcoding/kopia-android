...
class EdgeCaseFixtureCrossCompatibilityTest {
    ...
    fun policyPersistsInGoCreatedRepoAcrossReopen(): Unit = runBlocking {
    ...
    fun kotlinOpensFixtureAndListsSnapshots(): Unit = runBlocking {
        val storage = FilesystemBlobStorage(fixtureRepoDir)
        val repository = DirectRepositoryImpl.open(storage, fixturePassword)
        repo = repository
    ...
    fun kotlinRestoresGoEdgeCaseFixture(): Unit = runBlocking {
        val storage = FilesystemBlobStorage(fixtureRepoDir)
        val repository = DirectRepositoryImpl.open(storage, fixturePassword)
        repo = repository
    ...
    fun kotlinRestoresFixtureAndVerifiesContent(): Unit = runBlocking {
        val storage = FilesystemBlobStorage(fixtureRepoDir)
        val repository = DirectRepositoryImpl.open(storage, fixturePassword)
        repo = repository
    ...
    fun goAndKotlinSeeConsistentSnapshots(): Unit = runBlocking {
